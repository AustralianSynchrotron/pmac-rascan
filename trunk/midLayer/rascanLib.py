...
def set_pv_wait(epics_prefix, set_pv, set_val, get_pv, get_val, timout_sec=2, fault_pv='', do_no_status_check=False, do_verbose=False):
    msg = ''
    successful_ = False
    if len(set_pv) > 0:
    ...
    if not re.search('error',msg, re.IGNORECASE):
        successful_ = True

    return successful_, msg
# ---------------------------
...
#            if verbose_level > 5 : print('v1Diff_d1In_min {:f} > v1Diff_max {:f}'.format(v1Diff_d1In_min,v1Diff_max))
        ...
        if verbose_level > 4 :
            stats = ['d1Span-d1In*2','d1InHLM-d1In','tIn-tInLLM','d1OutHDM-d1Out','d2In/d2Step','dtLine_dv1Scan','dtLine_dv1Diff']
            print_stats(stats,locals(),indent_str=indent_stats)
